...
pub struct HistoryWidget<K: HistoryKind> {
    pub date: Edit<Actions, 8, 0, 1>,
    pub time: Edit<Actions, 6, 10, 0>,
    pub label: Label<Actions, 10, 0, 0>,
    pub value: Label<Actions, 8, 8, 1>,
...
impl<K: HistoryKind> HistoryWidget<K> {
    pub fn new() -> Self {
        Self {
            date: Edit::<Actions, 8, 0, 1>::new(""),
            time: Edit::<Actions, 6, 10, 0>::new(""),
            label: Label::<Actions, 10, 0, 0>::new("Расход за"),
            value: Label::<Actions, 8, 8, 1>::new(""),
    ...
    pub fn next_item(&mut self) -> bool {
        match self.items {
            DateTimeItems::None => match K::history_type() {
                HistoryType::Hour => {
