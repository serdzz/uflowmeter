...
| ~~10~~ | ~~History tests against fake EEPROM~~ | **Closed.** Added `tests/shims/zephyr/{drivers/eeprom.h,logging/log.h}` so `src/history_lib.hpp` compiles in the host test build, backed by `tests/fake_eeprom.{hpp,cpp}` (RAM-backed 128 KiB store, one-shot error injection, scoped install/uninstall). `tests/test_history_lib.cpp` exercises empty load, add/find round-trip, chained-ring non-overlap (`SIZE_ON_FLASH` invariant), wrap-around, gap-fill, large-jump reset, CRC-corruption restart, and read-error propagation — 10 cases. **Fixed a real bug in the embassy-port `find()`:** the `(size-1 - index)*ELEMENT_SIZE` formula only landed right indices for the fully-populated + offset==0 case; partial rings produced u32 underflow. Replaced with `time_of_last - step*ELEMENT_SIZE` starting from `(offset_of_last - 1 + SIZE) % SIZE`. Host tests now 62/62; binary size unchanged. |
