...
| UI state machine | `src/ui.rs`, `src/gui/*` | **Commits 1-4 of 6 done.** Navigation across all 19 screens + edit modes for CommType / SlaveAddress / Muster / Negative / SensorType (EEPROM-persisted) + DateTime field-stepping (RAM-backed) + HistoryWidget date picker (Hour / Day / Month with kind-specific field-stepping). Edit-commit triggers EEPROM save through DP wake/re-park. `src/datetime.{hpp,cpp}` provides per-field arithmetic with day-into-month cascading + leap year. `src/history.{hpp,cpp}` is a stub that always reports "no data" — UI exercises every code path end-to-end, but real values appear when the ring port lands and swaps the `query()` body. **Deferred**: Version easter-egg, Cyrillic CGRAM, AppRequest::DeepSleep/SystemReset/EnterCalibration handlers, real history ring lookup, RTC TR/DR datetime persistence. |
