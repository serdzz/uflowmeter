...
| RTC datetime persistence | `src/main.rs` `load_uptime_from_backup` | **Done.** `src/datetime.cpp` now reads/writes STM32L1 RTC TR (time) + DR (date) registers via direct CMSIS, BCD ↔ binary. Magic `0x44544D45` in RTC->BKP0R indicates the calendar is valid; `init()` loads live RTC on warm boot, falls back to 2024-01-01 baseline on cold VBAT or missing magic. RTC TR/DR auto-update from ck_spre (1 Hz tick from LSI); survives STOP and reset with VBAT. set() serialized via a mutex (UI dispatcher + shell SetDateUnix are both writers). |
