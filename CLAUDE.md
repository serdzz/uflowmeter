...
| UI state machine | `src/ui.rs`, `src/gui/*` | **Commits 1-3 of 6 done.** Navigation across all 19 screens + edit modes for CommType / SlaveAddress / Muster / Negative / SensorType (EEPROM-persisted) + DateTime field-stepping (RAM-backed). Edit-commit triggers EEPROM save through DP wake/re-park. `src/datetime.{hpp,cpp}` provides per-field arithmetic with day-into-month cascading + leap year. DateTime backing today: `k_uptime_get + RAM offset`; future commit wires STM32L1 RTC TR/DR + a backup-register baseline so wall-clock survives reboot. **Deferred**: HistoryWidget date picker, Version easter-egg, Cyrillic CGRAM, AppRequest::DeepSleep/SystemReset/EnterCalibration handlers. |
