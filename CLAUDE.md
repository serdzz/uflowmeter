...
| UI state machine | `src/ui.rs`, `src/gui/*` | **Commits 1-2 of 6 done.** Navigation across all 19 screens + edit modes for CommType / SlaveAddress / Muster / Negative / SensorType. Edit-commit triggers EEPROM save through DP wake/re-park (the first real Options write path on this port). Muster is memory-only (no Options field). **Deferred**: DateTime field-stepping, HistoryWidget date picker, Version easter-egg, Cyrillic CGRAM, AppRequest::DeepSleep/SystemReset/EnterCalibration handlers. |
