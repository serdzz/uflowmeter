...
| Modbus RTU | `src/modbus.rs`, `src/modbus_handler.rs` | **Commit 1 of 3 done.** `src/modbus.{hpp,cpp}` is the pure-logic codec — frame parse/build, CRC16-Modbus (poly 0xA001, distinct from Options' CRC16-CCITT-FALSE), 0x03/0x04/0x06/0x10 function dispatch, exception frames. No hardware deps, allocation-free. **Pending 2/3**: register handler (maps frames to Options/history/measurement). **Pending 3/3**: USART1 on-demand transport. |
