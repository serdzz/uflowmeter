...
| Modbus RTU | `src/modbus.rs`, `src/modbus_handler.rs` | **Commits 1-2 of 3 done.** Codec (`modbus.{hpp,cpp}`) + register handler (`modbus_handler.{hpp,cpp}`). Handler maps Modbus addresses to: Options bytes (Holding 0x0000-0x0039, R/W), live flow + accumulators (Holding 0x0064-0x006B, R), and flow data (Input 0x0000-0x0007). Write paths invoke Options::save through EEPROM DP wake/sleep — same pattern as UI edit commits. Handler refreshes slave_address from g_options each call so address changes via either UI or Modbus take effect immediately. **Pending 3/3**: USART1 on-demand transport — handler compiles in unused until then. |
