...
| UI state machine | `src/ui.rs`, `src/gui/*` | **Commits 1-5 of 6 done.** Navigation across all 19 screens + edit modes (EEPROM-persisted Set\*) + DateTime field-stepping (RAM-backed) + HistoryWidget date picker (Hour/Day/Month) + **Cyrillic glyph rendering** via `src/drivers/cyrillic.{hpp,cpp}` (~70-entry FONT table) + UTF-8 + per-frame CGRAM allocator in `src/ui/render.cpp`. ROM-lookalike substitution (А→A, Р→P, etc.) keeps CGRAM under the 8-slot budget on every screen. `src/datetime.{hpp,cpp}` provides per-field arithmetic. `src/history.{hpp,cpp}` is still a `nullopt`-returning stub. **Deferred to 6/6**: blink animation, Version easter-egg, AppRequest::DeepSleep/SystemReset/EnterCalibration handlers, real history ring lookup, RTC TR/DR datetime persistence, host-side tests. |
