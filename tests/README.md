...
| `src/options.cpp` | Pulls in `<zephyr/drivers/eeprom.h>`. CRC16-CCITT-FALSE function is testable but lives in the same file. Extracting to `crc.{hpp,cpp}` is the natural cleanup. |
| `src/history.cpp`, `src/history_lib.hpp` | Both consume the Zephyr eeprom_read/write API. A fake EEPROM device interface would let these run host-side; planned alongside the options decoupling. |
